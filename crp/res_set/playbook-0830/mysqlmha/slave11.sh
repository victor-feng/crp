...
sed -i "/NETWORKING=yes/a\HOSTNAME=$mysql2" /etc/sysconfig/network
hostname $mysql2

echo "$mysql2" > /usr/local/host_conf
#修改lvs_real的vip
...
#id=$(($ser_id-2))
num1=`cat /tmp/mysql.txt |sed -n '4p'|awk -F "." '{print $3}'`
num2=`cat /tmp/mysql.txt |sed -n '4p'|awk -F "." '{print $4}'`
serverid="$num2"3316
