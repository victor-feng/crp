...
class ResourceProviderTransitions(object):
    ...
    def tick_announce(self):
        ...
        if len(self.property_mapper) != 0 and self.property_mapper.keys()[0] not in ["kvm",self.resource_type]:
            Log.logger.info("1111111111333311444444444444111111112222222211111111111155555555555551--{}".format(
                self.property_mapper))
            Log.logger.info(
                "11111111113333114444444444441111111122222222111111111111555555555555519999--{}".format(
    ...
    def do_mysql_push(self):
        ...
        network_id = mysql.get("network_id", 0)
    ...
    def do_mongodb_push(self):
        ...
        if mongodb.get('quantity', {}) == 3:
            ...
            scp_cmd="ansible {ip} --private-key={dir}/mongo_script/old_id_rsa -m " \
                    "copy -a 'src={sh_path} dest=/tmp/ mode=777'".format(ip=ip,sh_path=sh_path ,dir=self.dir)
    ...
    def do_redis_push(self):
        ...
        if redis.get('quantity') == 2:
            vid, vip = self.create_vip_port(instance[0]['instance_name'],self.network_id)
            ip2 = instance[1]['ip']
    ...
    def mount_volume(self,ip,cluster_type):
        scp_cmd="ansible {ip} --private-key={dir}/mongo_script/old_id_rsa -m" \
                "copy -a 'src={dir}/volume.py dest=/tmp/'".format(ip=ip,dir=self.dir)
        exec_cmd="ansible {ip} --private-key={dir}/mongo_script/old_id_rsa " \
