...
def _create_resource_set(task_id=None, resource_list=None, compute_list=None):
    ...
    for compute in compute_list:
        ...
        if err_msg is None:
            ...
            result_inst_id_list = []
            # 删除全部
            _rollback_all(task_id, uop_os_inst_id_list, result_inst_id_list)
            uop_os_inst_id_list = []
...
# 回滚删除全部资源和容器
def _rollback_all(task_id, uop_os_inst_id_list, result_uop_os_inst_id_list):
    nova_client = OpenStack.nova_client
...
def _query_resource_set_status(task_id=None, result_list=None, uop_os_inst_id_list=None, req_dict=None):
    ...
    Log.logger.debug("Test Task Scheduler Class result_uop_os_inst_id_list object id is " + id(result_uop_os_inst_id_list).__str__() +
...
class ResourceSet(Resource):
    ...
    def post(cls):
        ...
        for resource in resource_list:
            ...
            version = resource.get('version')
            if ins_type == 'mysql':
                req_dict["mysql_inst_id"] = ins_id
            if ins_type == 'redis':
                req_dict["redis_inst_id"] = ins_id
            if ins_type == 'mongo':
                req_dict["mongodb_inst_id"] = ins_id
        ...
        for compute in compute_list:
            ...
            req_dict["cpu"] = cpu
            req_dict["memory"] = mem
            req_dict["container_inst_id"] = ins_id
        ...
        uop_os_inst_id_list = _create_resource_set(res_id, resource_list, compute_list)
