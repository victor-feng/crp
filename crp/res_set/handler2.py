...
class ResourceProviderTransitions2(object):
    ...
    def _rollback_all(
        ...
        Log.logger.debug("1111111111111111111111111111111111111111111111111{}".format(uop_os_inst_id_list))
        Log.logger.debug("222222222222222222222222222222222222222222222222222222{}".format(result_uop_os_inst_id_list))
        Log.logger.debug("333333333333333333333333333333333333333333333333{}".format(uop_os_inst_vol_id_list))
        if uop_os_inst_vol_id_list:
            ...
            Log.logger.debug("7777777777777777777777777777777777777777777{}".format(tmp_list))
        ...
        if replicas >= 1:
            ...
            if host_env == "docker":
                ...
                if self.set_flag == "res":
                    ...
                    #创建应用集群模板
                    deployment,err_msg = K8sDeployment.create_deployment_object(deployment_name,
                                                                           FILEBEAT_NAME,
                                                                           FILEBEAT_IMAGE_URL,
                                                                           filebeat_requests,
                                                                           filebeat_limits,
                    ...
                    if not err_msg:
                        if domain:
                            ...
                            if service_err_msg is None:
                                #创建ingress
                                ingress=K8sIngress.create_ingress_object(ingress_name,namespace,service_name,service_port,domain,lb_methods,domain_path)
                        ...
                        for instance in instances:
                            for deployment_info in deployment_info_list:
                                if instance.get('os_inst_id').lower() == deployment_info['deployment_name']:
                                    ...
                                    instance['physical_server'] = deployment_info.get("node_name","") + '@' + deployment_info.get("host_ip", "")
    ...
    API JOSN:
{
    ...
    if status == RES_STATUS_OK:
        for result_mapper in result_mappers_list:
            ...
            elif result_mapper.keys()[0] == resource_type:
                other=result_mapper.get(resource_type)
