...
def config():
    ...
    f_dst2 = re.sub('server  IpPort max_fails=1 fail_timeout=10s;', ips, f_dst1)
    ...
    # 执行nginx reload
    subprocess.Popen("sed -i 's/$//g' /usr/local/nginx/conf/servers_systoon/{}.format(domain)", shell=True, stdout=subprocess.PIPE)
